...
if($_GET["id"]){
    $id=$_GET["id"];
    $sql1="SELECT * FROM olaylar where id=".$id; 
    foreach ($baglan->query($sql1) as $row)
    {
        $id             = $row['id'];
        $cocukadi       = $row['cocukadi'];
        $olaytarihi     = $row['olaytarihi'];
        $olaybasligi    = $row['olaybasligi'];
        $olayaciklamasi = $row['olayaciklamasi'];
    }
}
?>
<!DOCTYPE html>
...
<br><br>
      ...
      <ul class="sidebar-menu" data-widget="tree">
        <li class=""><a href="index.php"><i class="fa fa-building"></i> <span>Anasayfa</span></a></li>
    <li class=""><a href="cocuklar.php"><i class="fa fa-child"></i> <span>Çocuklar</span></a></li>
		<li class="active"><a href="ani.php"><i class="fa fa-book"></i> <span>Anılar</span></a></li>
                ...
                <div class="box box-primary">
                    <div class="box-body">
                       ...
                       <nav aria-label="breadcrumb">
                          <ol class="breadcrumb">
                          <li class="breadcrumb-item"><a href="index.php">Ebeveyn Dasboard</a></li>
                            <li class="breadcrumb-item"><a href="ani.php">Anılar</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Anı Düzenle</li>
                          </ol>
                        </nav>

                <div class="tab-content">
                  <div role="tabpanel" class="tab-pane active">
                    <form method="POST" enctype="multipart/form-data" action="aniguncelleislem.php">
                      <div style="margin-top:10px">
                        <div class="col-xs-2">
                          <label>Kayıt Kodu</label>
                          <input type="text" class="form-control" name="kimlik"  autocomplete="off" value="<?php echo $id ?>"
                            required>
                        </div>
                        
                        <div class="col-xs-10">
                          <label>Çocuk Adı</label>
                          <input type="text" class="form-control" name="cocukadi"  autocomplete="off" value="<?php echo $cocukadi ?>"
                            required>
                        </div>
                      </div>
                        </div>
                      <div class="col-xs-6">
                          <label>Anı Tarihi</label>
                          <input type="date" class="form-control" name="olaytarihi" autocomplete="off" value="<?php echo $olaytarihi ?>"
                            required>
                        </div>
                        
                        <div class="col-xs-6">
                          <label>Anı Resmi</label>
                          <input type="file" class="form-control" name="olayresim"  autocomplete="off" disabled>
                        </div>
                      </div>
                  </div>
                  <div class="col-xs-12">
                          <label>Anı Başlığı</label>
                          <input type="text" class="form-control" name="olaybasligi"  autocomplete="off" value="<?php echo $olaybasligi ?>"
                            required>
                        </div>
                  <div class="col-xs-12">
                  <label style="margin-top:10px">Açıklama</label>
                  <textarea class="form-control" name="olayaciklamasi"><?php echo $olayaciklamasi ?></textarea>
                  </div><br></br>
                  <hr></br>
                  <div class="d-flex justify-content-end"><br><br><br>
                  &emsp;<button class="btn btn-primary">Kaydet</button><br><br>
                  </div>
                  </form>
                </div>
                        <!---------------------------------------------- /İçerikler Buraya --------------------------------------------->
